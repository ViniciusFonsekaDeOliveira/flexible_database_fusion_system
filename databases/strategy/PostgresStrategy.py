...
class PostgresStrategy(RelationalAdapter):
    ...
    def findOneByData(self, table: str, dataToSearch: dict, fields: str = "*"):
        clause_conditions, clause_values = getWhereClauseConditions(
            dataToSearch, self.placeholder_postgres)
    ...
    def insert(self, table: str, dataToInsert: dict):
        ...
        placeholders = ",".join(
            [f"{self.placeholder_postgres}"] * len(dataToInsert.keys()))
    ...
    def update(self, table: str, dataIdentifier: dict, updatedData: dict):
        columns_to_update = ",".join(
            [f"{column}={self.placeholder_postgres}" for column in updatedData.keys()])
        clause_conditions, clause_values = getWhereClauseConditions(
            dataIdentifier, self.placeholder_postgres)
    ...
    def close_connection(self):
        if self.connection:
            ...
            print("Conexão com o Postgres encerrada com sucesso!")
